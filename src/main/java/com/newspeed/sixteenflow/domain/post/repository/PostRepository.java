...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("""
        SELECT COUNT(p) FROM PostLike p 
    ...
    @Query("""
        ...
        where p.member.id In :followingIds 
        order by p.modifiedAt desc
    """)
    Page<Post> findPostsByFollowingIds(@Param("followingIds") List<Long> followingIds, Pageable pageable);

    @Query("""
        SELECT p FROM Post p
        WHERE (:startDate IS NULL OR p.modifiedAt >= :startDate)
        AND (:endDate IS NULL OR p.modifiedAt <= :endDate)
        AND (:keyword IS NULL OR p.content LIKE %:keyword%)
        ORDER BY p.modifiedAt DESC
    """)
    Page<Post> findAllWithSearch(
